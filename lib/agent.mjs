...
import { getPatientAssets } from './patient-assets.mjs';
...
function speakerWeight(fieldKey, speakerTag) {
  const doctorFields = new Set(['tbmedicalfinal', 'recommendations', 'dynamics', 'work-plan', 'objective-status', 'appointments']);
  const caregiverFields = new Set(['complaints', 'anamnesis', 'dynamics', 'planned-sessions', 'completed-sessions']);
  const patientFields = new Set(['complaints', 'anamnesis', 'tbmedicalfinal']);
  ...
  if (speakerTag === 'patient' && patientFields.has(fieldKey)) return 0.1;
...
function inferFactCandidates(appointment, text, speakerTag) {
  ...
  if (/(жалоб|жалуется|беспокоит|со слов родител|родител.*отмеча|обратил[аи]?сь по поводу|основн.*жалоб)/i.test(lower)) {
    facts.push({
      fact_id: makeId('fact-complaints'),
      source_type: 'transcript_chunk',
      speaker_tag: speakerTag,
      field_key: 'complaints',
      normalized_value: compactText(text),
      raw_evidence: text,
      confidence: 0.82 + speakerWeight('complaints', speakerTag)
    });
  }

  if (/(анамнез|ранее|с рождения|в течение|в анамнезе|из анамнеза|наблюдается с|по словам родител|со слов родител)/i.test(lower)) {
    facts.push({
      fact_id: makeId('fact-anamnesis'),
      source_type: 'transcript_chunk',
      speaker_tag: speakerTag,
      field_key: 'anamnesis',
      normalized_value: compactText(text),
      raw_evidence: text,
      confidence: 0.78 + speakerWeight('anamnesis', speakerTag)
    });
  }

  if (/(объективно|при осмотре|контакт|внимание|поведение|понимани[ея] инструкции|эмоциональ|речевая активность|память|мышление)/i.test(lower)) {
    facts.push({
      fact_id: makeId('fact-objective-status'),
      source_type: 'transcript_chunk',
      speaker_tag: speakerTag,
      field_key: 'objective-status',
      normalized_value: compactText(text),
      raw_evidence: text,
      confidence: 0.8 + speakerWeight('objective-status', speakerTag)
    });
  }

  if (/(назнач|план лечен|план работы|продолжить занятия|курс занятий|направить|домашн.*задани|домашн.*поддержк|коррекционн.*работ)/i.test(lower)) {
    facts.push({
      fact_id: makeId('fact-appointments'),
      source_type: 'transcript_chunk',
      speaker_tag: speakerTag,
      field_key: 'appointments',
      normalized_value: compactText(text),
      raw_evidence: text,
      confidence: 0.8 + speakerWeight('appointments', speakerTag)
    });
  }
...
function buildAllowedFieldContext(appointment) {
  const textFields = [
    { field_key: 'complaints', label: 'Жалобы' },
    { field_key: 'anamnesis', label: 'Анамнез' },
    { field_key: 'objective-status', label: 'Объективный статус' },
    { field_key: 'appointments', label: 'Назначения' },
...
async function inferOpenRouterPatches(runtime, appointment, patient, text, speakerTag) {
  const config = getOpenRouterConfig();
  if (!config.apiKey) {
    return { patches: [], facts: [], provider: 'heuristic', error: null };
  }

  const assets = getPatientAssets(runtime, patient?.patient_id || '');
  const templateAssets = assets.filter((a) => a.category === 'doctor-template').slice(0, 3);
  const historyAssets = assets.filter((a) => a.category === 'medical-history' || a.category === 'medical-document').slice(0, 3);
      ...
      messages: [
        {
          ...
          content: [
            ...
            'IMPORTANT: If "doctor_templates" are provided, strictly follow the writing style, professional terminology, and structural logic of those templates.',
            'Verify any mentioned facts against "patient_history" if provided.'
          ].join(' ')
        },
        {
          role: 'user',
          content: JSON.stringify({
            ...
            doctor_templates: templateAssets.map((a) => ({
              name: a.name,
              fields: a.template_fields
            })),
            patient_history: historyAssets.map((a) => ({
              name: a.name,
              summary: a.summary,
...
export function patchesToDomOperations(patches) {
  ...
  for (const patch of patches) {
    switch (patch.field_key) {
      case 'complaints':
        operations.push({ type: 'set-value', selector: '#tbComplaints', value: patch.value });
        break;
      case 'anamnesis':
        operations.push({ type: 'set-value', selector: '#tbAnamnesis', value: patch.value });
        break;
      case 'objective-status':
        operations.push({ type: 'set-value', selector: '#tbObjectiveStatus', value: patch.value });
        break;
      case 'appointments':
        operations.push({ type: 'set-value', selector: '#tbAppointments', value: patch.value });
        break;
...
export function buildHints(runtime, screenContext) {
  ...
  if (screenId === 'schedule') {
    ...
    return [
      ...
      },
      {
        ...
        severity: 'info',
        provenance: 'scheduler',
        message: `Доступно ${runtime.scheduleDays.length} календарных дней. Можно переключать график по дням, включая выходные.`,
        suggested_patches: []
      }
    ];
  }
  ...
  const missing = [];
  if (!appointment.inspection_draft.complaints_text) missing.push('Жалобы');
  if (!appointment.inspection_draft.anamnesis_text) missing.push('Анамнез');
  if (!appointment.inspection_draft.objective_status_text) missing.push('Объективный статус');
  if (!appointment.inspection_draft.appointments_text) missing.push('Назначения');
  ...
  const documentedFieldCount = [
    appointment.inspection_draft.complaints_text || appliedFieldKeys.has('complaints'),
    appointment.inspection_draft.anamnesis_text || appliedFieldKeys.has('anamnesis'),
    appointment.inspection_draft.objective_status_text || appliedFieldKeys.has('objective-status'),
    appointment.inspection_draft.appointments_text || appliedFieldKeys.has('appointments'),
    appointment.inspection_draft.conclusion_text || appliedFieldKeys.has('tbmedicalfinal'),
    appointment.inspection_draft.supplemental?.recommendations || appliedFieldKeys.has('recommendations'),
    appointment.inspection_draft.supplemental?.dynamics || appliedFieldKeys.has('dynamics')
  ].filter(Boolean).length;
  const requiredLooksComplete = documentedFieldCount >= 4 && missing.length <= 2;
...
export async function ingestTranscript(runtime, { appointmentId, sessionId, text, speakerTag }) {
  ...
  const openRouterResult = await inferOpenRouterPatches(runtime, appointment, patient, normalizedText, inferredSpeaker);
...
export function injectDraftPatches(runtime, appointmentId, incomingPatches = [], options = {}) {
  const appointment = getAppointmentById(runtime, appointmentId);
  if (!appointment) throw new Error('Appointment not found');

  const normalized = (incomingPatches || [])
    .map((patch) => ({
      ...clone(patch),
      patch_id: patch.patch_id || makeId(patch.section_key || patch.field_key || 'patch'),
      provenance: patch.provenance || options.provenance || 'patient_preset',
      confidence: Number(patch.confidence || options.confidence || 0.84),
      status: 'suggested'
    }))
    .filter((patch) => patch.field_key || patch.section_key);

  appointment.draft_state.draft_patches = options.replaceExisting
    ? normalized
    : mergeDraftPatches(appointment.draft_state.draft_patches || [], normalized);
  appointment.draft_state.draft_status = appointment.draft_state.draft_patches.length ? 'ready_for_apply' : 'idle';
  appointment.draft_state.updated_at = nowIso();
  appointment.draft_state.last_preview = buildPreviewFromDraftState(runtime, appointmentId);
  ...
  return {
    draftState: clone(appointment.draft_state),
    preview: clone(appointment.draft_state.last_preview)
  };
}
