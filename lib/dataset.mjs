...
function buildArchivePatient(index, historyItem, template) {
  ...
  let fullName = chunks[0] || rawLabel || `РџР°С†РёРµРЅС‚ РёР· Р°СЂС…РёРІР° ${index + 1}`;
  ...
  return {
    ...
    baseline_conclusion: diagnosisLabel
      ? `РСЃС‚РѕСЂРёСЏ РёРјРїРѕСЂС‚РёСЂРѕРІР°РЅР° РёР· Р°СЂС…РёРІР°: ${diagnosisLabel}.`
      : 'РСЃС‚РѕСЂРёСЏ РёРјРїРѕСЂС‚РёСЂРѕРІР°РЅР° РёР· Р°СЂС…РёРІР° РїР°С†РёРµРЅС‚Р°.',
...
export function seedRuntimeState(artifacts) {
  ...
  const seededAppointments = new Map([
    ['0-0', { patient: linkedPrimaryPatient, status: 'completed' }],
    ['1-0', { patient: linkedPrimaryPatient, status: 'scheduled' }]
  ]);
  const scheduleDays = businessDays.map((day, dayIndex) => {
    const date = day.toISOString().slice(0, 10);
    const slots = [0, 1, 2, 3].map((slotIndex) => {
      const seededAppointment = seededAppointments.get(`${dayIndex}-${slotIndex}`) || null;
      const patient = seededAppointment?.patient || null;
      const slotProvider = providers.find((item) => item.provider_id === patient?.assigned_provider_id) || primaryProvider;
      ...
      return {
        ...
        provider_id: slotProvider.provider_id,
        status,
