...
const ALLOWED_DURATION_MIN = [30, 40];
...
export function getAvailablePsychologistSlots(psychologist, date, durationMin, takenSlots = []) {
  if (!ALLOWED_DURATION_MIN.includes(durationMin)) {
    throw new Error(`Psychologist sessions support ${ALLOWED_DURATION_MIN.join(' or ')}-minute slots only.`);
  }
...
export function generatePsychologistSchedule(input) {
  ...
  const durationMin = ALLOWED_DURATION_MIN.includes(Number(input?.durationMin)) ? Number(input.durationMin) : DEFAULT_DURATION_MIN;
  ...
  if (input?.durationMin != null && !ALLOWED_DURATION_MIN.includes(Number(input.durationMin))) {
    throw new Error(`Psychologist sessions support ${ALLOWED_DURATION_MIN.join(' or ')}-minute slots only.`);
  }
