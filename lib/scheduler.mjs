  ...
  return [
    {
      ...
      full_name: 'БАТЫРГАЛИЕВА ЖАННА АЛЕКСАНДРОВНА',
      work_start: '09:00',
      work_end: '18:00',
      lunch_start: '13:00',
      lunch_end: '14:00',
...
export function buildPsychologistsFromRuntime(runtime) {
  ...
  const mappedProviders = providers
    .filter((provider) => /психолог/i.test(String(provider.specialty || provider.schedule_name || '')))
    .map((provider) => ({
...
function selectDistributedDates(workingDays, sessionCount) {
  if (sessionCount >= workingDays.length) return workingDays.slice();
  if (sessionCount === 1) return [workingDays[0]];

  const indices = [];
  const used = new Set();

  for (let step = 0; step < sessionCount; step += 1) {
    let index = Math.round((step * (workingDays.length - 1)) / (sessionCount - 1));

    while (used.has(index) && index < workingDays.length - 1) index += 1;
  ...
  return indices.sort((left, right) => left - right).map((index) => workingDays[index]);
}
...
export function generatePsychologistSchedule(input) {
  ...
  const workingDays = generateNext9WorkingDays(startDate);
  const targetDates = selectDistributedDates(workingDays, sessionCount);
  const takenSlots = [];
  const days = [];
  const unassigned = [];
  ...
  for (const date of targetDates) {
    ...
    if (!selected) {
      unassigned.push({
        date,
        durationMin,
        reason: 'No free psychologist slot available on this working day.'
      });
      continue;
    }
