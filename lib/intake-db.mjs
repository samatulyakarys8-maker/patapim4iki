...
export function createIntakeStore({
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS intakes (
      ...
      conversation_step TEXT NOT NULL DEFAULT 'iin',
      patient_fio TEXT,
      iin TEXT,
      phone TEXT,
      main_complaint TEXT,
  ...
  function createOrActivateDoctorIntake({ waId = '', contactName = '', doctorId = '' }) {
    ...
    if (existing) {
      const nextStep = existing.conversation_step === 'fio' && !existing.patient_fio && !existing.iin
        ? 'iin'
        : existing.conversation_step;
      db.prepare('UPDATE intakes SET contact_name = ?, conversation_step = ?, updated_at = ? WHERE intake_id = ?')
        .run(contactName || existing.contact_name || '', nextStep, nowIso(), existing.intake_id);
