...
function buildAdvisorContext(runtime, { appointmentId, question, screenContext = {} }) {
  ...
  return {
    ...
    patient: {
      ...
      history_refs: [
        ...(patient.history_refs || []),
        ...getPatientAssets(runtime, patient.patient_id)
           .filter((a) => a.category !== 'doctor-template')
           .map((a) => a.extracted_text || a.text_excerpt || a.summary)
      ].filter(Boolean).slice(0, 5),
      template_sections: [
        ...(patient.medical_template?.sections || []).slice(0, 12).map((section) => ({
        ...
        ...getPatientAssets(runtime, patient.patient_id)
           .filter((a) => a.category === 'doctor-template')
           .map((a) => ({
              section_key: 'custom_template',
              title: `Шаблон: ${a.name}`,
              kind: 'uploaded',
              extracted_fields: a.template_fields
           }))
      ]
    },
